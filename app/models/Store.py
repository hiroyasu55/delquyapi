...
class Store(object):
    ...
    def entity_to_dict(self, data):
        ...
        if type(data) is dict:
            for key in data:
                data[key] = self.entity_to_dict(data[key])
        elif type(data) is list:
            data = list(map(lambda d: self.entity_to_dict(d), data))
    ...
    def get(self, kind, id):
        entity = None
        if not id:
            raise Exception('get: id not defined.')

        entitiy_key = client.key(kind, id)
        entity = client.get(entitiy_key)
        return entity

    def add(self, kind, object, id=None):
        if id:
        ...
        entity.update({'created': self.now_string(), 'updated': self.now_string()})
        client.put(entity)
        entity['id'] = entity.key.id or id
        logger.debug('add: kind={},id={}'.format(kind, id))
